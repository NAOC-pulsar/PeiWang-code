...
PATHF=0
 ...
 while [ -n "$1" ]; do        
    ...
    -dec) DEC=$2;;
    -path) PATHF=$2;;
    -h) HELP=1;echo "#  HELP :";echo " ";echo "-dec : DEC (dd:mm:ss)";echo "Path    : Path of datafile folder";echo "Example :$. filename2pointing.sh -dec 30:00:00 -path /home/data/20170901";echo " ";;
    esac
    shift
 done

ls ${PATHF}/*.fits > filelist.txt
tot=`cat filelist.txt | wc -l`

for DATASF in `cat filelist.txt` ; do
let "FLAG = ${FLAG} + 1"
MJD=`readfile ${PATHF} | grep STT | awk '{print $6}'`
echo "${FLAG}/${tot}    ${PATHF}   MJD=${MJD}"
python - ${MJD} ${DEC} ${FLAG} ${PATHF} <<EOF >> output.list
